...
#include "lwip/dns.h"
#define GATTC_TAG "GATTC_MULTIPLE_CONNECT"
...
static esp_err_t mqtt_event_handler_cbA(esp_mqtt_event_handle_t event)
{
    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            esp_mqtt_client_subscribe(clientA, "v1/devices/me/attributes", 1);
            break;
        ...
        case MQTT_EVENT_DATA:
            ...
            printf("%s\n", number_A);
            esp_ble_gattc_write_char( globalIf_A,
...
static void mqtt_app_start(void)
{
    esp_mqtt_client_config_t mqtt_cfgA = {
        ...
        //.port= 1883,
        .username="jJyLIdSQHMjDGMHV0hJ4",
       // .password=""
    };
    esp_mqtt_client_config_t mqtt_cfgB = {
        .uri = "mqtt://thingsboard.cloud/api/v1/$ACCESS_TOKEN",
        //.port= 1883,
        .username="IKnbFaa8oAWuXCe4jJSx",
       // .password=""
    };
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(GATTC_TAG, "ESP_GATTC_NOTIFY_EVT, Receive notify A value:");
        mystring_a=(char *)p_data->notify.value;
        a=atof(mystring_a);
        
        sprintf(REQUEST_A,"{\"Temperature A\":%f} \n",a);
        esp_mqtt_client_publish(A, "v1/devices/me/telemetry",REQUEST_A , 0, 1, 0);

   /*      esp_mqtt_client_publish(A, "v1/devices/me/telemetry", REQUEST_A, 0, 1, 0);
esp_mqtt_client_publish(B, "v1/devices/me/telemetry", REQUEST_B, 0, 1, 0);
A và B là hai MQTT client khác nhau (giả định A và B được khởi tạo bằng esp_mqtt_client_init với cấu hình riêng).
Nếu mỗi client MQTT sử dụng Access Token khác nhau (trong cấu hình esp_mqtt_client_config_t), ThingsBoard sẽ phân biệt đây là hai thiết bị khác nhau, vì mỗi Access Token sẽ liên kết với một thiết bị cụ thể trên ThingsBoard.
Điều kiện phân biệt:

MQTT client A dùng Access Token Token_A.
MQTT client B dùng Access Token Token_B. */

        ESP_LOGI(GATTC_TAG, "REQUEST_A: %s", REQUEST_A);
...
static void gattc_profile_b_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ESP_LOGI(GATTC_TAG, "ESP_GATTC_NOTIFY_EVT, Receive notify B value:");
        mystring_b=(char *)p_data->notify.value;
         b=atof(mystring_b);
        sprintf(REQUEST_B,"{\"Temperature B\":%f}\n",b);
        esp_mqtt_client_publish(B, "v1/devices/me/telemetry",REQUEST_B , 0, 1, 0);
         ESP_LOGI(GATTC_TAG, "REQUEST_B: %s", REQUEST_B);
          printf("REQUEST_B: %s\n", REQUEST_B);
...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
           ...
           {
             esp_mqtt_client_publish(A, "esp/attributes","{\"Status\":false}" , 0, 1, 0);
        ...
        case ESP_GAP_SEARCH_INQ_CMPL_EVT:
            ...
            uint32_t duration = 5;
            printf("Dang quet lai trong 5 giay \n");
            esp_ble_gap_start_scanning(duration); 
