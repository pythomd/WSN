...
void ssd1306_display_text_x2(SSD1306_t * dev, int page, char * text, int text_len, bool invert)
{
	...
	for (uint8_t nn = 0; nn < _text_len; nn++) {
		...
		for (uint8_t yy = 0; yy < 2; yy++)	{ // for each group of 8 pixels high (y-direction)
			...
			if (invert) ssd1306_invert(image, 18);
			if (dev->_flip) ssd1306_flip(image, 18);
			if (dev->_address == SPIAddress) {
				spi_display_image(dev, page+yy, seg, image, 18);
			} else {
				i2c_display_image(dev, page+yy, seg, image, 18);
			}
			memcpy(&dev->_page[page+yy]._segs[seg], image, 18);
		}
		seg = seg + 18;
	}
}
